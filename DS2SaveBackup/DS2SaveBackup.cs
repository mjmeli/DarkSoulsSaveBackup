...
{
    public partial class DS2SaveBackup : Form
    {
        ...
        private void DS2SB_OnShow(object sender, EventArgs e)
        {
            backingUp = false;
            saveLoc = new SaveLoc();
            if (Properties.Settings.Default.buDir == "" || Properties.Settings.Default.buDir == @"C:\Users\Michael\AppData\Roaming\DarkSoulsII\0110000103c637eb\Backups")
            {
                buLoc = new BackUpLoc(saveLoc.FolderPath, true);
            }
            else
            {
                buLoc = new BackUpLoc(Properties.Settings.Default.buDir, false);
            }

            tbSaveLoc.Text = saveLoc.FolderPath;
            tbSaveLoc.Select(tbSaveLoc.Text.Length, 0);
            cbDel.Checked = Properties.Settings.Default.delete;
            decimal buFreq = Properties.Settings.Default.buFreq;
            decimal keepLast = Properties.Settings.Default.keepLast;
            udFreq.Value = buFreq;
            udDel.Value = keepLast;
        ...
        private void bSaveLoc_Click(object sender, EventArgs e)
        {
            if (saveFolBrowser.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                ...
                {
                    this.updateSaveLoc(saveFolBrowser.SelectedPath);
                    Properties.Settings.Default.saveDir = saveFolBrowser.SelectedPath;
        ...
        private void bBULoc_Click(object sender, EventArgs e)
        {
            if (buFolBrowser.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                ...
                Properties.Settings.Default.buDir = buFolBrowser.SelectedPath;
                Properties.Settings.Default.Save();
            }
        }
        ...
        private void updateBULoc(string s)
        {
            ...
            Properties.Settings.Default.buDir = newPath;
            Properties.Settings.Default.Save();
        }
        ...
        private void backup()
        {
            ...
            foreach (SaveFile s in saveLoc.SaveFiles)
            {
                ...
                {
                    string newFileName = s.FileName.ToUpper() + String.Format("({0})", s.LastBU + 1);
                    newFileName = System.IO.Path.Combine(buLoc.FolderPath, newFileName);
                    string oldFileName = System.IO.Path.Combine(saveLoc.FolderPath, s.FileName);
                    System.IO.File.Copy(oldFileName, newFileName, true);
                    s.LastBU = s.LastBU + 1;
        ...
        private void btReset_OnClick(object sender, EventArgs e)
        {
            Properties.Settings.Default.saveDir = "";
            Properties.Settings.Default.Save();
            SaveLoc temp = new SaveLoc();
            this.updateSaveLoc(temp.FolderPath);
        }

        // values of up downs changed - save settings
        private void UpDowns_ValueChanged(object sender, EventArgs e)
        {
            Properties.Settings.Default.buFreq = udFreq.Value;
            Properties.Settings.Default.keepLast = udDel.Value;
            Properties.Settings.Default.Save();
            Properties.Settings.Default.Save();
        }
    }
